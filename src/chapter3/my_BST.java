...
//my
    private Value get(Node x, Key key) {
        ...
        if(x==null) return null; //x==null or x.child==null
...
//my
    ...
    // Return key of rank k. there is k nodes which is smaller than the node
    
    //my
    private Node select(Node x, int k) {
        if(x==null) return null;
        if(x.left.size==k) return x;
        else if(x.left.size>k) return select(x.left, k);
        else return select(x.right,k-x.left.size-1);
    ...
    private int rank(Key key, Node x) {
        if (x == null) return 0; 
        int cmp = key.compareTo(x.key); 
        if(cmp==0) return x.left.size;
        else if(cmp>0) return x.left.size+1+rank(key,x.right);
  ...
  /*************************************************************************
    ...
    ***************************************************************************/
    ...
    public static void main(String[] args) { 
    	my_BST<String, Integer> st = new my_BST<String, Integer>();
